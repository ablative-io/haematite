...
//! missing-node discovery, and the wire codec now live in the ungated
//! [`crate::sync_codec`] module so they compile on wasm. This module keeps every
//! `crate::sync::protocol::*` path stable by re-exporting them, and still owns the
//! native-only beamr transport glue (`send_*_via_beamr`,
//! `register_beamr_sync_handler`) via the `wire` submodule below.
...
pub use crate::sync_codec::{
    ...
    RootExchangeRequest, RootExchangeResponse, ShardSyncRequest, SyncDecision, SyncError, SyncPlan,
    SyncStats, TargetNodeReader, TargetNodeRequest, TargetNodeResponse, TargetNodeSummary, WriteAck,
    WriteId, WriteProposal, find_missing_nodes, plan_sync,
};
pub use wire::{
