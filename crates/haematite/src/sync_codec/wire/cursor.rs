...
impl<'a> MessageCursor<'a> {
    ...
    pub fn read_u8(&mut self) -> Result<u8, SyncError> {
    ...
    pub fn read_bool(&mut self) -> Result<bool, SyncError> {
        match self.read_u8()? {
            0 => Ok(false),
            1 => Ok(true),
    ...
    pub fn read_u32_as_usize(&mut self) -> Result<usize, SyncError> {
    ...
    pub fn read_u32(&mut self) -> Result<u32, SyncError> {
        let bytes = self.read_exact(4)?;
        let mut value = [0_u8; 4];
        value.copy_from_slice(bytes);
    ...
    pub fn read_u64(&mut self) -> Result<u64, SyncError> {
    ...
    pub fn read_usize(&mut self) -> Result<usize, SyncError> {
        let bytes = self.read_exact(8)?;
        let mut value = [0_u8; 8];
        value.copy_from_slice(bytes);
    ...
    pub fn read_shard_id(&mut self) -> Result<ShardId, SyncError> {
        self.read_usize()
    }

    pub fn read_optional_hash(&mut self) -> Result<Option<Hash>, SyncError> {
        match self.read_u8()? {
            0 => Ok(None),
    ...
    pub fn read_optional_target_summary(&mut self) -> Result<Option<TargetNodeSummary>, SyncError> {
        match self.read_u8()? {
            0 => Ok(None),
    ...
    pub fn read_len_prefixed_bytes(&mut self) -> Result<Vec<u8>, SyncError> {
        let len = self.read_usize()?;
        self.read_exact(len).map(<[u8]>::to_vec)
    }

    pub fn read_sync_node_id(&mut self) -> Result<SyncNodeId, SyncError> {
    ...
    pub fn read_ballot(&mut self) -> Result<Ballot, SyncError> {
        let counter = self.read_u64()?;
        let node = self.read_sync_node_id()?;
        Ok(Ballot::new(counter, node))
    }

    pub fn read_stamp(&mut self) -> Result<Stamp, SyncError> {
        let epoch = self.read_ballot()?;
        let seq = self.read_u64()?;
        Ok(Stamp::new(epoch, seq))
    }

    pub fn read_optional_ballot(&mut self) -> Result<Option<Ballot>, SyncError> {
        match self.read_u8()? {
            0 => Ok(None),
    ...
    pub fn read_write_id(&mut self) -> Result<WriteId, SyncError> {
    ...
    pub fn read_optional_duration(&mut self) -> Result<Option<Duration>, SyncError> {
        match self.read_u8()? {
            0 => Ok(None),
            1 => {
    ...
    pub fn read_ack_outcome(&mut self) -> Result<AckOutcome, SyncError> {
        match self.read_u8()? {
    ...
    pub fn read_hash(&mut self) -> Result<Hash, SyncError> {
    ...
    pub fn read_exact(&mut self, len: usize) -> Result<&'a [u8], SyncError> {
        let end = self
            .position
            .checked_add(len)
    ...
    pub const fn finish(&self) -> Result<(), SyncError> {
